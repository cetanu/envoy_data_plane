...
class RedisProxy(betterproto.Message):
    ...
    # error. A :ref:`catch-all   route<envoy_api_field_config.filter.network.redi
    # s_proxy.v2.RedisProxy.PrefixRoutes.catch_all_route>`   would have retrieved
    # the key from that cluster instead. See the :ref:`configuration section
    # <arch_overview_redis_configuration>` of the architecture overview for
    # recommendations on configuring the backing clusters.
...
class RedisProtocolOptions(betterproto.Message):
    ...
    yped_extension_protocol_options>`, keyed by the name `envoy.redis_proxy`.
    """
