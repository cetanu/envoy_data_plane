...
class StatsSink(betterproto.Message):
    ...
    # stats sink. The built-in stats sinks are: * :ref:`envoy.statsd
    # <envoy_api_msg_config.metrics.v3.StatsdSink>` * :ref:`envoy.dog_statsd
    # <envoy_api_msg_config.metrics.v3.DogStatsdSink>` *
    # :ref:`envoy.metrics_service
    # <envoy_api_msg_config.metrics.v3.MetricsServiceConfig>` *
    # :ref:`envoy.stat_sinks.hystrix
    # <envoy_api_msg_config.metrics.v3.HystrixSink>` Sinks optionally support
...
class TagSpecifier(betterproto.Message):
    ...
    # "^cluster\.((.+?)\.)"   } Note that the regex will remove ``foo_cluster.``
    # making the tag extracted name ``cluster.upstream_rq_timeout`` and the tag
    # value for ``envoy.cluster_name`` will be ``foo_cluster`` (note: there will
    # be no ``.`` character because of the second capture group). Example 2. a
    # stat name ``http.connection_manager_1.user_agent.ios.downstream_cx_total``
    # and two tag specifiers: .. code-block:: json   [     {       "tag_name":
    # "envoy.http_user_agent",       "regex":
    # "^http(?=\.).*?\.user_agent\.((.+?)\.)\w+?$"     },     {       "tag_name":
    # "envoy.http_conn_manager_prefix",       "regex": "^http\.((.*?)\.)"     }
    # ] The two regexes of the specifiers will be processed in the definition
    # order. The first regex will remove ``ios.``, leaving the tag extracted name
...
class StatsdSink(betterproto.Message):
    """
    Stats configuration proto schema for built-in *envoy.statsd* sink. This
    sink does not support tagged metrics. [#extension: envoy.stat_sinks.statsd]
    """
...
class DogStatsdSink(betterproto.Message):
    """
    Stats configuration proto schema for built-in *envoy.dog_statsd* sink. The
    sink emits stats with `DogStatsD
...
class MetricsServiceConfig(betterproto.Message):
    """
    Metrics Service is configured as a built-in *envoy.metrics_service*
    :ref:`StatsSink <envoy_api_msg_config.metrics.v3.StatsSink>`. This opaque
