...
class EnvoyInternalAddress(betterproto.Message):
    ...
    # <envoy_v3_api_field_config.listener.v3.Listener.name>` of the destination
    # internal listener.
...
class BindConfig(betterproto.Message):
    ...
    # <envoy_v3_api_field_config.cluster.v3.UpstreamBindConfig.source_address>`
...
class DnsResolverOptions(betterproto.Message):
    ...
    # Use TCP for all DNS queries instead of the default protocol UDP. Setting
    # this value causes failure if the
    # ``envoy.restart_features.use_apple_api_for_dns_lookups`` runtime value is
    # true during server startup. Apple's API only uses UDP for DNS resolution.
...
class DnsResolutionConfig(betterproto.Message):
    ...
    # system resolvers (e.g., /etc/resolv.conf) will be used. Setting this value
...
class QuicProtocolOptions(betterproto.Message):
    ...
    # minimum/default as *initial_stream_window_size*. NOTE: 16384 (2^14) is the
    # minimum window size supported in Google QUIC. We only support increasing
    # the default window size now, so it's also the minimum.
...
class KeepaliveSettings(betterproto.Message):
    ...
    # determine if the connection is dead.
...
class Http2ProtocolOptions(betterproto.Message):
    ...
    # due to flood mitigation. The default limit is 10000. NOTE: flood and abuse
    # mitigation for upstream connections is presently enabled by the
    ...
    # connections due to flood mitigation. The default limit is 1000. NOTE: flood
    # and abuse mitigation for upstream connections is presently enabled by the
    # `envoy.reloadable_features.upstream_http2_flood_checks` flag.
    ...
    # an empty payload and no end stream flag. The default limit is 1. NOTE:
    # flood and abuse mitigation for upstream connections is presently enabled by
    # the `envoy.reloadable_features.upstream_http2_flood_checks` flag.
    ...
    # NOTE: flood and abuse mitigation for upstream connections is presently
    # enabled by the `envoy.reloadable_features.upstream_http2_flood_checks`
    # flag.
    ...
    # estimate available bandwidth require at least 2. NOTE: flood and abuse
    # mitigation for upstream connections is presently enabled by the
    # `envoy.reloadable_features.upstream_http2_flood_checks` flag.
