...
class OutlierDetection(betterproto.Message):
    ...
    # is capped by :ref:`max_ejection_time<envoy_api_field_config.cluster.v3.Outl
    # ierDetection.max_ejection_time>`. Defaults to 30000ms or 30s.
    ...
    # nvoy_api_field_config.cluster.v3.OutlierDetection.base_ejection_time>` for
    # more information. If not specified, the default value (300000ms or 300s) or
    # :ref:`base_ejection_time<envoy_api_field_config.cluster.v3.OutlierDetection
    # .base_ejection_time>` value is applied, whatever is larger.
...
class Cluster(betterproto.Message):
    ...
    # of :ref:`upstream_http_protocol_options <envoy_api_field_extensions.upstrea
    # ms.http.v3.HttpProtocolOptions.upstream_http_protocol_options>` in the
    # :ref:`http_protocol_options
    # <envoy_api_msg_extensions.upstreams.http.v3.HttpProtocolOptions>` message.
    # upstream_http_protocol_options can be set via the cluster's :ref:`extension
    # _protocol_options<envoy_api_field_config.cluster.v3.Cluster.typed_extension
    # _protocol_options>`. See ref:`upstream_http_protocol_options <envoy_api_fie
    # ld_extensions.upstreams.http.v3.HttpProtocolOptions.upstream_http_protocol_
    # options>` for example usage.
    ...
    # favor of :ref:`common_http_protocol_options <envoy_api_field_extensions.ups
    # treams.http.v3.HttpProtocolOptions.common_http_protocol_options>` in the
    # :ref:`http_protocol_options
    # <envoy_api_msg_extensions.upstreams.http.v3.HttpProtocolOptions>` message.
    # common_http_protocol_options can be set via the cluster's :ref:`extension_p
    # rotocol_options<envoy_api_field_config.cluster.v3.Cluster.typed_extension_p
    # rotocol_options>`. See ref:`upstream_http_protocol_options <envoy_api_field
    # _extensions.upstreams.http.v3.HttpProtocolOptions.upstream_http_protocol_op
    # tions>` for example usage.
    ...
    # :ref:`http_protocol_options
    # <envoy_api_msg_extensions.upstreams.http.v3.HttpProtocolOptions>` message.
    # http_protocol_options can be set via the cluster's :ref:`extension_protocol
    # _options<envoy_api_field_config.cluster.v3.Cluster.typed_extension_protocol
    # _options>`. See ref:`upstream_http_protocol_options <envoy_api_field_extens
    # ions.upstreams.http.v3.HttpProtocolOptions.upstream_http_protocol_options>`
    # for example usage.
    ...
    # <envoy_api_msg_extensions.upstreams.http.v3.HttpProtocolOptions>` message.
    # http2_protocol_options can be set via the cluster's :ref:`extension_protoco
    # l_options<envoy_api_field_config.cluster.v3.Cluster.typed_extension_protoco
    # l_options>`. See ref:`upstream_http_protocol_options <envoy_api_field_exten
    # sions.upstreams.http.v3.HttpProtocolOptions.upstream_http_protocol_options>
    # ` for example usage.
...
class ClusterPreconnectPolicy(betterproto.Message):
    ...
    # services. Preconnecting will only be done if the upstream is healthy. For
    ...
    # in use, and 50 connections preconnected. This might be a useful value for
    ...
    # connections, 5 preconnected connections would be in the queue in case of
    ...
    # upstream-rtt latency penalty waiting for streams to be preconnected. This
    # is limited somewhat arbitrarily to 3 because preconnecting too aggressively
    # can harm latency more than the preconnecting helps.
    ...
    # would be picked next and pre-establishing a connection. For example if
    # preconnecting is set to 2 for a round robin HTTP/2 cluster, on the first
    # incoming stream, 2 connections will be preconnected - one to the first
    # upstream for this cluster, one to the second on the assumption there will
    # be a follow-up stream. Preconnecting will be limited to one preconnect per
    # configured upstream in the cluster. If this value is not set, or set
    # explicitly to one, Envoy will fetch as many connections as needed to serve
    # streams in flight, so during warm up and in steady state if a connection is
    # closed (and per_upstream_preconnect_ratio is not set), there will be a
    # latency hit for connection establishment. If both this and preconnect_ratio
    # are set, Envoy will make sure both predicted needs are met, basically
    # preconnecting max(predictive-preconnect, per-upstream-preconnect), for each
