...
# sources: envoy/service/ext_proc/v3alpha/external_processor.proto
...
class ProcessingRequest(betterproto.Message):
    ...
    # or asynchronous mode. If false, then the server must either respond with
    # exactly one ProcessingResponse message or close the stream. If true,
    # however, then the server must not respond with an additional message,
    # although it may still close the stream. The choice of synchronous or
    # asynchronous mode can be chosen in the filter configuration.
    async_mode: bool = betterproto.bool_field(1)
    # Information about the HTTP request headers, as well as peer info and
    # additional properties. If "response_required" is set, the server must send
    ...
    # A chunk of the HTTP request body. If "response_required" is set, the server
    ...
    # The HTTP trailers for the request path. If "response_required" is set, the
    # server must send back a TrailerResponse message or close the stream.
    request_trailers: "HttpTrailers" = betterproto.message_field(6, group="request")
    # The HTTP trailers for the response path. If "response_required" is set, the
    # server must send back a TrailerResponse message or close the stream.
...
class ProcessingResponse(betterproto.Message):
    ...
    mode_override: "___extensions_filters_http_ext_proc_v3_alpha__.ProcessingMode" = (
...
class HttpHeaders(betterproto.Message):
    ...
    # "response_attributes" list in the configuration. Each entry in the list is
    # populated from the standard :ref:`attributes <arch_overview_attributes>`
...
class CommonResponse(betterproto.Message):
    ...
    # HttpBody request, header mutations will only take effect if the headers
    # were not already sent further on the filter chain, which happens only if
    # the current processing mode for the body is BUFFERED or BUFFERED_PARTIAL.
    ...
    # body chunk that was sent with that request. If responding to an HttpHeaders
    # request, then a new body may be added to the request if this message is
    # returned along with the CONTINUE_AND_REPLACE status.
...
class ExternalProcessorBase(ServiceBase):
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            "/envoy.service.ext_proc.v3alpha.ExternalProcessor/Process": grpclib.const.Handler(
                self.__rpc_process,
...
from ....extensions.filters.http.ext_proc import (
    v3alpha as ___extensions_filters_http_ext_proc_v3_alpha__,
)
