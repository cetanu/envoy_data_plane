...
class HttpService(betterproto.Message):
    ...
    lowed_upstream_headers>` for details. Additionally, the filter may add
    additional headers to the client's response. See
    :ref:`allowed_client_headers_on_success <envoy_api_field_extensions.filters
    .http.ext_authz.v4alpha.AuthorizationResponse.allowed_client_headers_on_suc
    cess>` for details. On other authorization response statuses, the filter
    will not allow traffic. Additional headers metadata as well as body may be
    added to the client's response. See :ref:`allowed_client_headers <envoy_api
    _field_extensions.filters.http.ext_authz.v4alpha.AuthorizationResponse.allo
    wed_client_headers>` for details. [#next-free-field: 9]
    """
...
class AuthorizationResponse(betterproto.Message):
    ...
    # <envoy_api_msg_type.matcher.v4alpha.ListStringMatcher>`. is set,
    # authorization response headers that have a correspondent match will be
    # added to the client's response when the authorization response itself is
    ...
    allowed_client_headers_on_success: "_____type_matcher_v4_alpha__.ListStringMatcher" = betterproto.message_field(
        4
    )
