...
# sources: envoy/extensions/filters/http/ext_proc/v3alpha/ext_proc.proto, envoy/extensions/filters/http/ext_proc/v3alpha/processing_mode.proto
...
class ExternalProcessor(betterproto.Message):
    ...
    # [#not-implemented-hide:] By default, if the gRPC stream cannot be
    # established, or if it is closed prematurely with an error, the filter will
    # fail. Specifically, if the response headers have not yet been delivered,
    # then it will return a 500 error downstream. If they have been delivered,
    # then instead the HTTP stream to the downstream client will be reset. With
    # this parameter set to true, however, then if the gRPC stream is prematurely
    # closed or could not be opened, processing continues without error.
    failure_mode_allow: bool = betterproto.bool_field(2)
    # [#not-implemented-hide:] Specifies default options for how HTTP headers,
    # trailers, and bodies are sent. See ProcessingMode for details.
    ...
    # [#not-implemented-hide:] Specifies the timeout for each individual message
    # sent on the stream and when the filter is running in synchronous mode.
    # Whenever the proxy sends a message on the stream that requires a response,
    # it will reset this timer, and will stop processing and return an error
    # (subject to the processing mode) if the timer expires. There is no timeout
    # when the filter is running in asynchronous mode. Default is 200 ms.
...
class ExtProcPerRoute(betterproto.Message):
    """
    [#not-implemented-hide:] Extra settings that may be added to per-route
    configuration for a virtual host or cluster.
    """
...
class ExtProcOverrides(betterproto.Message):
    """
    [#not-implemented-hide:] Overrides that may be set on a per-route basis
    """
