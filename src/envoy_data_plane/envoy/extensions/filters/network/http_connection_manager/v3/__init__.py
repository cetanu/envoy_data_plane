...
class HttpConnectionManager(betterproto.Message):
    ...
    # provider <envoy_api_msg_config.trace.v3.Tracing>`.
    ...
    # <envoy_api_field_config.route.v3.RouteAction.idle_timeout>`. Even on a
    ...
    # filters.network.http_connection_manager.v3.HttpConnectionManager.stream_idl
    # e_timeout>` applies. Each time an encode/decode event for headers or data
    # is processed for the stream, the timer will be reset. If the timeout fires,
    ...
    # does not apply to this corner case. If the :ref:`overload action
    ...
    # api_enum_value_config.overload.v3.ScaleTimersOverloadActionConfig.TimerType
    # .HTTP_DOWNSTREAM_STREAM_IDLE>`. Note that it is possible to idle timeout
    ...
    # onnection_manager.v3.HttpConnectionManager.use_remote_address>` will also
    ...
    # field_extensions.filters.network.http_connection_manager.v3.HttpConnectionM
    # anager.forward_client_cert_details>` is APPEND_FORWARD or SANITIZE_SET and
    # the client connection is mTLS. It specifies the fields in the client
    ...
    # p_connection_manager.v3.HttpConnectionManager.use_remote_address>` is true
    # and represent_ipv4_remote_address_as_ipv4_mapped_ipv6 is true and the
    ...
    # :ref:`listener's<envoy_api_field_config.listener.v3.Listener.address>`
    ...
    # :ref:`domains<envoy_api_field_config.route.v3.VirtualHost.domains>` match
    # set to `example`. Defaults to `false`. Note that port removal is not part
    # of `HTTP spec <https://tools.ietf.org/html/rfc3986>`_ and is provided for
    # convenience. Only one of `strip_matching_host_port` or
    # `strip_any_host_port` can be set.
    strip_matching_host_port: bool = betterproto.bool_field(39)
    # Determines if the port part should be removed from host/authority header
    # before any processing of request by HTTP filters or routing. The port would
    # be removed only if request method is not CONNECT. This affects the upstream
    # host header as well. Without setting this option, incoming requests with
    # host `example:443` will not match against route with
    # :ref:`domains<envoy_api_field_config.route.v3.VirtualHost.domains>` match
    # set to `example`. Defaults to `false`. Note that port removal is not part
    ...
    # voy_api_msg_extensions.filters.network.http_connection_manager.v3.PathNorma
    # lizationOptions>` for details.
...
class HttpConnectionManagerUpgradeConfig(betterproto.Message):
    ...
    # <envoy_api_field_config.route.v3.RouteAction.upgrade_configs>` as
...
class HttpConnectionManagerPathNormalizationOptions(betterproto.Message):
    ...
    <envoy_api_field_config.route.v3.RouteAction.regex_rewrite>` or
    :ref:`prefix_rewrite
    <envoy_api_field_config.route.v3.RouteAction.prefix_rewrite>`) will apply
    to the *:path* header destined for the upstream. Note: access logging and
    tracing will show the original *:path* header.
    """
    ...
    # <envoy_api_msg_type.http.v3.PathTransformation.Operation.NormalizePathRFC39
    # 86>`. When not specified, this value may be overridden by the runtime
...
class ResponseMapper(betterproto.Message):
    ...
    # A per mapper `body_format` to override the :ref:`body_format <envoy_v3_api_
    # field_extensions.filters.network.http_connection_manager.v3.LocalReplyConfi
    # g.body_format>`. It will be used when this mapper is matched.
...
class ScopedRoutes(betterproto.Message):
    ...
    # _msg_extensions.filters.network.http_connection_manager.v3.ScopedRoutes.Sco
    # peKeyBuilder>` in this message.
    ...
    # work.http_connection_manager.v3.ScopedRoutes.ScopeKeyBuilder>` in this
    # message.
...
class ScopedRoutesScopeKeyBuilder(betterproto.Message):
    ...
    :ref:`Key<envoy_api_msg_config.route.v3.ScopedRouteConfiguration.Key>`
    objects assembled from :ref:`ScopedRouteConfiguration<envoy_api_msg_config.
    route.v3.ScopedRouteConfiguration>` messages distributed via SRDS (the
    ...
    ection_manager.v3.ScopedRoutes.scoped_route_configurations_list>`. Upon
    ...
    e.v3.RouteConfiguration>`) to use for the request.
    """
    ...
    # RouteConfiguration<envoy_api_msg_config.route.v3.ScopedRouteConfiguration>`
    # . A missing fragment during comparison will make the key invalid, i.e., the
...
class HttpFilter(betterproto.Message):
    ...
    # <envoy_api_msg_extensions.common.matching.v3.ExtensionWithMatcher>` with
    ...
    # filter must reject the config. [#not-implemented-hide:]
