...
class HttpConnectionManager(betterproto.Message):
    ...
    # A timeout for idle requests managed by the connection manager. The timer is
    # activated when the request is initiated, and is disarmed when the last byte
    # of the request is sent upstream (i.e. all decoding filters have processed
    # the request), OR when the response is initiated. If not specified or set to
    # 0, this timeout is disabled.
    ...
    # Comparison <https://tools.ietf.org/html/rfc3986#section-6>` for details of
    ...
    # is not part of `HTTP spec <https://tools.ietf.org/html/rfc3986>` and is
...
class HttpConnectionManagerUpgradeConfig(betterproto.Message):
    ...
    # documented in the :ref:`upgrade documentation <arch_overview_websocket>`.
