...
class HttpConnectionManager(betterproto.Message):
    ...
    # does not apply to this corner case. Note that it is possible to idle
    # timeout even if the wire traffic for a stream is non-idle, due to the
    # granularity of events presented to the connection manager. For example,
    # while receiving very large request headers, it may be the case that there
    # is traffic regularly arriving on the wire while the connection manage is
    # only able to observe the end-of-headers event, hence the stream may still
    # idle timeout. A value of 0 will completely disable the connection manager
    # stream idle timeout, although per-route idle timeout overrides will
    # continue to apply.
