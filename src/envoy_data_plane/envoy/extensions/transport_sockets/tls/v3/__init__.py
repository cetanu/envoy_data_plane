...
class CertificateValidationContext(betterproto.Message):
    ...
    # <envoy_api_msg_type.matcher.v3.StringMatcher>`. For example if the
...
class UpstreamTlsContext(betterproto.Message):
    ...
    # api_field_extensions.transport_sockets.tls.v3.CertificateValidationContext.
    # trusted_ca>` to enable   verification.
...
class DownstreamTlsContext(betterproto.Message):
    ...
    # tensions.transport_sockets.tls.v3.DownstreamTlsContext.session_ticket_keys>
    # ` or :ref:`session_ticket_keys_sds_secret_config <envoy_api_field_extension
    # s.transport_sockets.tls.v3.DownstreamTlsContext.session_ticket_keys_sds_sec
    # ret_config>`. If this config is set to false and no keys are explicitly
    # configured, the TLS server will issue TLS session tickets and
...
class CommonTlsContext(betterproto.Message):
    ...
    # n_manager.v3.HttpConnectionManager.codec_type>` parameter in the HTTP
...
class SpiffeCertValidatorConfig(betterproto.Message):
    ...
    ext>`. - :ref:`allow_expired_certificate <envoy_api_field_extensions.transp
    ort_sockets.tls.v3.CertificateValidationContext.allow_expired_certificate>`
    to allow expired certificates. - :ref:`match_subject_alt_names <envoy_api_f
    ield_extensions.transport_sockets.tls.v3.CertificateValidationContext.match
    _subject_alt_names>` to match **URI** SAN of certificates. Unlike the
    default validator, SPIFFE validator only matches **URI** SAN (which equals
    to SVID in SPIFFE terminology) and ignore other SAN types.
    """
