...
# sources: envoy/api/v2/auth/cert.proto
...
class TlsParameters(betterproto.Message):
    ...
    # Maximum TLS protocol version. By default, it's ``TLSv1_3`` for servers in
    # non-FIPS builds, and ``TLSv1_2`` for clients and for servers using
...
class TlsCertificate(betterproto.Message):
    ...
    certificate_chain: core.DataSource = betterproto.message_field(1)
    # The TLS private key.
    private_key: core.DataSource = betterproto.message_field(2)
    ...
    password: core.DataSource = betterproto.message_field(3)
    # [#not-implemented-hide:]
    ocsp_staple: core.DataSource = betterproto.message_field(4)
    # [#not-implemented-hide:]
    signed_certificate_timestamp: List[core.DataSource] = betterproto.message_field(5)
...
class TlsSessionTicketKeys(betterproto.Message):
    ...
    keys: List[core.DataSource] = betterproto.message_field(1)
...
class CertificateValidationContext(betterproto.Message):
    ...
    trusted_ca: core.DataSource = betterproto.message_field(1)
    ...
    # of the specified matches. .. attention::   Subject Alternative Names are
    # easily spoofable and verifying only them is insecure,   therefore this
    # option must be used together with :ref:`trusted_ca
    # <envoy_api_field_auth.CertificateValidationContext.trusted_ca>`.
    match_subject_alt_names: List[matcher.StringMatcher] = betterproto.message_field(9)
    ...
    crl: core.DataSource = betterproto.message_field(7)
...
class CommonTlsContextCombinedCertificateValidationContext(betterproto.Message):
    # How to validate peer certificates.
    default_validation_context: "CertificateValidationContext" = betterproto.message_field(
        1
    )
    # Config for fetching validation context via SDS API.
    validation_context_sds_secret_config: "SdsSecretConfig" = betterproto.message_field(
...
class DownstreamTlsContext(betterproto.Message):
    ...
    session_ticket_keys_sds_secret_config: "SdsSecretConfig" = betterproto.message_field(
        5, group="session_ticket_keys_type"
    )
...
class SdsSecretConfig(betterproto.Message):
    ...
    sds_config: core.ConfigSource = betterproto.message_field(2)
